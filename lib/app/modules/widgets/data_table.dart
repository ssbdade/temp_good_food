...
class DataTableView extends StatefulWidget {
  const DataTableView({Key? key, required this.data}) : super(key: key);
...
class _DataTableViewState extends State<DataTableView> {
  ...
  Widget build(BuildContext context) {
    return DataTable(
      ...
      columns: dataColumn,
      headingRowHeight: 80,
      rows: List.generate(DataBasesData.dataBasesData.length, (index) => DataRow(
        cells: List.generate(
            widget.data[index].genList().length+2,
                (i) {
              if(i == 0) {
                return DataCell(
                    Center(
                ...
                return DataCell(
                    Center(
                      child: SizedBox(
                        child: Text(widget.data[index].genList()[1],
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
